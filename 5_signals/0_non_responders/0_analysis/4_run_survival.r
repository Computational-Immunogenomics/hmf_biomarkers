...
ready <- readRDS(paste0(SHARE_DIR, "biomarkers_survival_ready.Rds"))$ready
...
combination_features <- readRDS(paste0(SHARE_DIR, "biomarkers_survival_ready.Rds"))$combination_features
features <- c(categorical_features, combination_features)
...
for( i in cohorts$cohortGo ){
  ...
  tmp_oo <- scanner(  y = "Surv(daysToPfsEvent, pfsEvent)", features, covariates = "", df = "tmp", mod = "coxph", cohort = i)
  if( nrow(tmp_oo) > 0){
